...
import { CinemaService } from 'src/cinema/services/cinema.service';
...
import { MovieDetails } from 'src/movie/dto/movie-details';
  ...
  export class PlanningController {
    constructor(private readonly planningService: PlanningService, private readonly movieService: MovieService) {}
    ...
    async listPlanningsByCinema(@Query() planningFilters: PlanningFilters) {
      ...
      const uniqueMovies = plannings.map(planning => planning.movieId).filter((value, index, self) => {
    ...
    @Get("bymovie")
    async listPlanningsByMovie(@Query() planningFilters: PlanningFiltersByMovie ):Promise<MoviePlanning[]>  {
    ...
    async setup(@Body() plannings:ScrappedMoviePlanning[]) {
        const uniqueMovies = plannings.map(planning => planning.title).filter((value, index, self) => {
            return self.indexOf(value) === index;
        });
